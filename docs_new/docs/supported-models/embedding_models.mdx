...
<Warning>
Most decoder-style embedding models require `--is-embedding`, and some require `--trust-remote-code`. Encoder-only and sequence-classification architectures are recognized automatically. `google/embeddinggemma-300m` is also detected automatically.
</Warning>
