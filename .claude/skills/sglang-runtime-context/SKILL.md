...
pretends to be `server_args`. Stubbing a *single accessor* is a different thing and
stays allowed when the case isolates one predicate and publishing the whole config
would be heavier than the test — `test_attention_patching.py`
(`mamba_extra_buffer_lazy_enabled`) and `test_kimi_k3_vision.py`
(`configured_tp_size`) are the two that do it deliberately. If the case is about the
value a config carries, publish; if it is about one branch of one helper, stub the
helper and say so.
...
5. **Global config read ratchet** (`test_global_config_read_ratchet.py`): baselines are
   **0** for both the direct `get_server_args().field` and the alias form (function-local
   or module-level, plus the `getattr(..., "field")` spelling of each). Exempt by owner
